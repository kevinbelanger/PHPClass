					...
					foreach($arrGroupToProcess as $groupDn)
					{
						if($this->exists($groupDn))
						{
							if($this->_rootDse->getServerType() == Zend_Ldap_Node_RootDse::SERVER_TYPE_ACTIVEDIRECTORY)
							{
								if($mode == "add")
								{
									$entity = $this->_ldap->getEntry($groupDn, array("member"));
									Zend_Ldap_Attribute::setAttribute($entity, "member", $searchResult["dn"], true);
								}
								elseif($mode == "delete")
								{
									$entity = $this->_ldap->getEntry($groupDn, array("member"));
									Zend_Ldap_Attribute::removeFromAttribute($entity, "member", $searchResult["dn"]);
								}

								if($this->update($groupDn, $entity))
								{
							...
							elseif($this->_rootDse->getServerType() == Zend_Ldap_Node_RootDse::SERVER_TYPE_EDIRECTORY)
							{
								if($mode == "add")
								{
									$userEntity = $this->_ldap->getEntry($searchResult["dn"]);
									Zend_Ldap_Attribute::setAttribute($userEntity, "securityequals", $groupDn, true);
									Zend_Ldap_Attribute::setAttribute($userEntity, "groupmembership", $groupDn, true);
									
									$groupEntity = $this->_ldap->getEntry($groupDn);
									Zend_Ldap_Attribute::setAttribute($groupEntity, "member", $searchResult["dn"]);
									Zend_Ldap_Attribute::setAttribute($groupEntity, "equivalenttome", $searchResult["dn"]);
								}
								elseif($mode == "delete")
								{
									$userEntity = $this->_ldap->getEntry($searchResult["dn"]);
									Zend_Ldap_Attribute::removeFromAttribute($userEntity, "securityequals", $groupDn);
									Zend_Ldap_Attribute::removeFromAttribute($userEntity, "groupmembership", $groupDn);

									$groupEntity = $this->_ldap->getEntry($groupDn);
									Zend_Ldap_Attribute::removeFromAttribute($groupEntity, "member", $searchResult["dn"]);
									Zend_Ldap_Attribute::removeFromAttribute($groupEntity, "equivalenttome", $searchResult["dn"]);
								}
								
								if($this->update($searchResult["dn"], $userEntity) && $this->update($groupDn, $groupEntity))
								{
									$arrResult[$groupDn]["error"] = false;
								}
								else
