...
class Mail
{
	...
	function __construct($arrProprieteMail = null)
	{
		...
		{
			if(array_key_exists('emailFromName', $arrProprieteMail))
			{
				$this->__set("emailFromName", $arrProprieteMail["emailFromName"]);
			}

			if(array_key_exists('emailFrom', $arrProprieteMail))
			{
				$this->__set("emailFrom", $arrProprieteMail["emailFrom"]);
			}

			if(array_key_exists('emailTo', $arrProprieteMail))
			{
				$this->__set("emailTo", $arrProprieteMail["emailTo"]);
			}
			...
			if(array_key_exists('emailObject', $arrProprieteMail))
			{
				$this->__set("emailObject", $arrProprieteMail["emailObject"]);
			}

			if(array_key_exists('email', $arrProprieteMail))
			{
				$this->__set("email", $this->creerCorpMessage($arrProprieteMail["email"]));
			}
	...
	{
		if($this->_arrProprieteMail["emailFromName"] != "")
		{
			$strEntete = "From: " . $this->_arrProprieteMail["emailFromName"] . " <" . $this->_arrProprieteMail["emailFrom"] . ">" . $this->_strSautLigne;
			$strEntete .= "Reply-To: " . $this->_arrProprieteMail["emailFromName"] . " <" . $this->_arrProprieteMail["emailFrom"] . ">" . $this->_strSautLigne;
		}
		else
		{
			$strEntete = "From: " . $this->_arrProprieteMail["emailFrom"] . $this->_strSautLigne;
			$strEntete .= "Reply-To: " . $this->_arrProprieteMail["emailFrom"] . $this->_strSautLigne;
		}
	...
	{
		$strMessageCourriel = $this->_arrProprieteMail["email"];
		...
		if(mail(implode(", ", $this->_arrProprieteMail["emailTo"]), $this->_arrProprieteMail["emailObject"], $strMessageCourriel, $this->_strMailEntetes))
		{
			return true;
		}
		else
